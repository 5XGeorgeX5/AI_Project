...
class GomokuBoard:
    ...
    def count_patterns(self, key: tuple) -> int:
        ...
        return (
            2 * ((3 ** vals[0]))
            if vals[0] != 0
            else 0 + ((3 ** vals[1]) if vals[1] != 0 else 0)
        )
    ...
    def heuristic2(self) -> tuple[int, int]:
            ...
            if cutting:
                for winning_position in self.__winning_positions_dict[i]:
                    ...
                    for cell in winning_position:
                        total += self.board[cell]
                    if (
                        total % TileType.WHITE != 0
                        and total % TileType.BLACK != 0
                        and winning_position in self.__winning_positions
                    ):
